...
### Step 6. QA 전문가 최종 검증 (필수 — 커밋/배포 전)
- **QA전문가** 호출하여 `agent/qa-expert/skills.md` 체크리스트 기반 검증:
  1. 기능 로직 정확성 (정상/엣지/에러 케이스) 점검.
  2. 인증·인가 및 RLS 정책 준수 확인.
  3. 반응형 UI 및 사용자 경험 검증.
  4. QA 검증 리포트 작성 후 "배포 가능" 판정 시 커밋 진행.

### Step 7. 보안 전문가 검토 (개인정보/신규 API 추가 시 필수)
- **보안전문가** 호출하여 `agent/security-expert/skills.md` 기반 검토:
  1. 신규 API 엔드포인트의 입력값 검증 및 인증 확인.
  2. 개인정보 처리 항목 및 저장 방식 검토.
  3. 환경변수 노출 여부 확인.
  4. 보안 검토 리포트 작성 후 "이상 없음" 판정 시 배포 진행.
