...
export default function AddTaskModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-indigo-950/30 backdrop-blur-sm transition-opacity" onClick={onClose}></div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                                <Calendar size={12} className="text-indigo-400" /> 시기 (D-Day offset)
                            </label>
                        ...
                        <div>
                            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                                <Edit2 size={12} className="text-indigo-400" /> 할 일 (Task)
                            </label>
                            <input
                                type="text"
                                name="title"
                        ...
                        <div>
                            <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                                <FileText size={12} className="text-indigo-400" /> 메모 (Memo)
                            </label>
                            <textarea
                                name="memo"
                                className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl py-3 px-4 text-sm text-gray-700 outline-none transition-all focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-400/10 placeholder:text-gray-300 resize-none h-24"
